...
def get_play_list(start_url, cid, quality):
    entropy = 'rbMCKn@KuamXWlPMoJGsKcbiJKUfkPF_8dABscJntvqhRSETg'#接口密钥
    appkey, sec = ''.join([chr(ord(i) + 2) for i in entropy[::-1]]).split(':')#排序解析密钥
    params = 'appkey=%s&cid=%s&otype=json&qn=%s&quality=%s&type=' % (appkey, cid, quality, quality)#构造参数
    chksum = hashlib.md5(bytes(params + sec, 'utf8')).hexdigest()#加密密钥
    url_api = 'https://interface.bilibili.com/v2/playurl?%s&sign=%s' % (params, chksum)#构造url
    headers = {
        'Referer': start_url,  # 注意加上referer
        'User-Agent': 'Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/55.0.2883.87 Safari/537.36'
    }
    # print(url_api)
    html = requests.get(url_api, headers=headers).json()#获取视频下载链接
...
def down_video(video_list, title, start_url, page):
    ...
    currentVideoPath = os.path.join(sys.path[0], 'bilibili_video', title)  # 当前目录作为下载目录 只是拼接目录
    for i in video_list:#循环取出video下载链接
        opener = urllib.request.build_opener()  # 构建请求头handlers有关
        # 请求头
        opener.addheaders = [
        ...
        # 创建文件夹存放下载的视频
        if not os.path.exists(currentVideoPath):#判断目录是否存在os.path.exists()
            os.makedirs(currentVideoPath)#创建目录os.mkedirs()
        # 开始下载
        if len(video_list) > 1:#使用urlretrieve取回数据
